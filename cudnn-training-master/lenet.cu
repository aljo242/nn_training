...
int main(int argc, char **argv)
{
    ...
    */
    ...
    std::vector<int> my_times(FLAGS_iterations);
    ...
    for (int iter = 0; iter < FLAGS_iterations; ++iter)
    {
        ...
        my_times[iter] = std::chrono::duration_cast<std::chrono::nanoseconds>(stop - start).count(); 
    ...
    double average_no_update = (double)(std::accumulate(my_times.begin(), my_times.end(), 0) / my_times.size() / 1000000.0f);

    printf("Total Average time: %f ms\n", std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() / 1000.0f / FLAGS_iterations);
    printf("Average time No Update Weights: %f ms\n", average_no_update);
